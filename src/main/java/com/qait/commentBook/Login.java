...
public class Login {
	...
	public Response login(@FormParam("email") String email,@FormParam("password") String password) throws URISyntaxException{
		...
		try{  
    		...
    		ResultSet rs=stmt.executeQuery("select email,password from userAccount");
    		...
    		while(rs.next())  
    		{
    			...
    			if(email.equals(rs.getString(1)) && password.equals(rs.getString(2)))
    			{
    				...
    				URI location = new URI("http://localhost:12015/commentBook/home.html");
...
//    						+ " <a href='http://localhost:8080/commentBook/login.html'>Click here</a> to login again"
//    						+ "</body></html>";
//    				
//    				
//    			}
		...
		output="<html></body>"
				+ "<h1>INCORRECT USER NAME OR PASSWORD.</h1><br><br>"
				+ " <a href='http://localhost:12015/fakebook/login.html'>Click here</a> to login again"
				+ "</body></html>";
		URI location = new URI("http://localhost:8080/commentBook/login.html");
